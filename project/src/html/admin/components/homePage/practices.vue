<template>
	<section>
		...
		<paperitem 
			v-for="paper in organPracticeList"  
		...
		/>
		<p v-if="organPracticeList.length<=0" class="no-info"> 没有数据... </p>
	</section>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				'organPracticeList': state => {
					return state.organization.organPracticeList
				},
		...
		methods: {
			...
			toGetPracticesByOrganId() {
				this.spinShow = true
				this.getPracticesByOrganId({
